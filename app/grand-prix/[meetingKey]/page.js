...
export default function GrandPrix({ params }) {
  ...
  return (
    <div>
      <h1>{meetingData[0].location} Grand Prix</h1>
      <h2>{meetingData[0].country_name}</h2>
      <h3>
        Event Duration: {formatDate(firstSessionStart)} to{" "}
        {formatDate(lastSessionEnd)}
      </h3>
      <div>
        {meetingData.map((session) => (
          <div key={session.session_key}>
            ...
            <p>
              Start: {formatDate(session.date_start)} | End:{" "}
